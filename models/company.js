...
class Company {
  ...
  static async get(query) {
    // Constructing the SQL query to fetch data about the company and its jobs
    ...
    // Adding WHERE clause based on query parameters
    queryString = queryString + sqlCompWhereQuery(query);
    // Grouping the results by company handle and ordering by company name
    queryString = queryString + ` GROUP BY c.handle ORDER BY name`;
    const companiesRes = await db.query(queryString);
    // Handling the case where no company is found
