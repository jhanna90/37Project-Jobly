...
// Adding ensureAdmin ensures that the authenticated user has administrative privileges to make changes.
...
router.post(
  ...
  async function (req, res, next) {
    try {
      ...
      // Extracting job ID from URL parameters and converting to integer
      const jobId = parseInt(req.params.id);
      // Creating a new application object with username and job ID
      const newApp = { username, jobId };
      // Validating the new application object against the schema
      const validator = jsonschema.validate(newApp, appNewSchema);
      if (!validator.valid) {
        const errs = validator.errors.map((e) => e.stack);
        throw new BadRequestError(errs);
      }
      // Applying for the job with the new application
